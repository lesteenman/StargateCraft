...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float par1, boolean par2, int par3, int par4)
    {
		...
		if(!DimensionAPI.overlayList.isEmpty())
		{
			...
			{
				((IRenderOverlay)DimensionAPI.overlayList.get(overlays)).renderGameOverlay(mc, i, j);
			}
		}
    ...
    private void renderPortalOverlay(float par1, int par2, int par3)
    {
		...
		{
			texture = currentPortal.getOverlayTexture();
			textureOverride = currentPortal.getOverlayTextureOverride();
		}
        ...
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, mc.renderEngine.getTexture(textureOverride));
        float f = (float)(texture % 16) / 16F;
        float f1 = (float)(texture / 16) / 16F;
        float f2 = (float)(texture % 16 + 1) / 16F;
        float f3 = (float)(texture / 16 + 1) / 16F;
        Tessellator tessellator = Tessellator.instance;
        tessellator.startDrawingQuads();
